...
class first_screen:
    def __init__(self, width, height):
        ...
        self.login_button = button("Login", width // 2 - self.button_width // 2, height // 2 - 150, self.button_width, self.button_height,
                              red, 0.5)
        self.signup_button = button("Sign up", width // 2 - self.button_width // 2, height // 2 + 100, self.button_width,
                               self.button_height, red, 0.5)
        self.title_text = animated_text("Welcome to comp-sci revision game", width//2, height//2-250, black, 40, 1.5)

    def handle_screen(self):
        if self.login_button.is_clicked(event):
            return login_screen(width, height)
        elif self.signup_button.is_clicked(event):
            return signup_screen(width, height)
